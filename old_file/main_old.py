...
class MainWindow(QObject):
    def __init__(self):
        ...
        with open("LISTfromDeviceCatalog.json") as fp:
            self.getResponse= json.load(fp)
    
        gymLoader = GymLoader(self.getResponse)
    ...
    addElem = Signal(int,dict)


    @Slot(str)
    def addNewElement(self,stra):
        print(stra)
        self.addElem.emit(1,{"cost": 5.95, "name":"Pizza"})   
    ...
    def selectGym(self,gymName):
        ...
        self.roomList = []
        self.deviceList = self.getResponse["palestre"]
    ...
    def addNewGym(self,gymName):
        #Bisogna specificare la posizione della palestra nella lista di cui bisogna aggiornare lo stato!!!!
        self.gymModelList.changeStatus("closed", 2)
        self.gymModelList.changeStatus("problems", 1)
        print(f"Add new Gym: {gymName}")
        self.gymName = gymName
        
        ##########
        # Aggiornare vista palestre,stanze e device solo ad inserimento palestra ultimato!!!!
        #self.roomList = []
        #self.deviceList = []
        #self.roomModelList = mr.RoomModelList(self.roomList)
        #self.engine.rootContext().setContextProperty('roomModelList', self.roomModelList)
...
if __name__ == "__main__":
    ...
    # Set Context
#    main.engine.rootContext().setContextProperty("pippo", main)
